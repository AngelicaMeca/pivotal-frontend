...
- **Zoom** (al pie de cada cuadro con gráfico, desde el 24-sep-2026): amplía ESE cuadro a pantalla completa, con los controles que lo gobiernan adentro y más grandes. El motivo del pedido son los controles: en el tablero van chicos porque todo tiene que entrar en una pantalla. Es `<dialog>` nativo con `showModal()` —Escape, foco atrapado y fondo inerte sin dependencias—, y **mueve el nodo real del panel** adentro y lo devuelve al cerrar; clonarlo dejaría dos instancias de ECharts sobre el mismo `data-grafico`. Al abrir y al cerrar repinta por el mismo camino que el resize de ventana, porque el mapa se calcula con el tamaño de su caja. Los controles que viven afuera del panel viajan como copia, lo que es seguro porque `comun.js` sincroniza todos los controles que comparten `data-filtro`. La palabra es la de JC, que ya lo tenía dibujado en su maqueta de pasturas. Se declara panel por panel (`paneles.<id>.acciones`), nunca a nivel de tablero. Con un cuadro ampliado, "Generar PDF" cierra el zoom antes de imprimir: el PDF es siempre el tablero completo. La caja del diálogo deja un marco de 2vmin de fondo alrededor: ese marco es el ÚNICO pedazo del `<dialog>` que queda a la vista y es lo que hace que el clic afuera cierre (sin él, `.zoom-caja` tapaba el 100% y la condición `target === dialogo` no se cumplía nunca). La animación de apertura es un FLIP que se calcula **contra la caja, no contra la ventana**: con el marco ya no miden lo mismo.
- **Cruzar datos** (adentro del Zoom, desde el 24-sep-2026): dos controles apagados por defecto para superponer **otro valor del filtro principal** del cuadro (otro cultivo, otro producto, otro año) y/o una **segunda medida**. Va solo en el zoom: el tablero es la maqueta literal de JC y no lleva selectores de comparación; al cerrar el zoom se apaga sola. Lo declara el spec panel por panel (`paneles.<id>.comparacion`) y solo lo aceptan las formas de serie temporal (`site_build.FORMAS_QUE_COMPARAN`): en un mapa, un anillo o un ranking una segunda serie no se lee sin mentir. El cruce por valor **no agrega payload**: es una segunda clave de combinación sobre los combos que ya están (si el JSON está partido por ese filtro, el navegador baja la otra partición). La segunda medida sí viaja, en `panel.medidas_extra`. Reglas de JC que lo condicionan: las dos series nunca se combinan en un número; misma unidad = un eje, distinta unidad = dos ejes rotulados; el color de la serie comparada es **otro paso de la misma rampa** (`presentacion.Colores.comparacion`) más punteado, nunca una paleta nueva; la leyenda nombra cada serie con su valor y el título dice que hay comparación (plantillas del spec). El PDF sigue siendo el tablero completo y la barra del zoom lo aclara.
